            ...
            Parallel.ForEach(urls, url =>
            {
                ...
                Parallel.ForEach(validBlocks, validBlock =>
                {
                    ...
                    Parallel.ForEach(validBlock.Events, e =>
                    {
                        ...
                        var evento = new SportEvent(e.Id, e.LeagueId, e.RegionId, leagueName, startDate, e.Participants[0].Name, e.Participants[1].Name, new(odds.Selections[0].Price, odds.Selections[1].Price, odds.Selections[2].Price, more2and5odds?.Selections[0].Price ?? 0.0d, more2and5odds?.Selections[1].Price ?? 0.0d), webSiteName, $"{urlHome}{e.Url}");
                        events.Add(evento);
                    });
                });
